...
const PersonForm = (props) => {
  ...
  const addPerson = (event) => {
    ...
    if (
      ...
      if (
        window.confirm(
          `${person.name} is already added to phonebook. Do you want to replace the old number with the new one?`
        )
      ) {
  ...
  const Part = (props) => {
    ...
    return (
      <li key={part.id}>
        ...
        <DeleteButton id={part.id} name={part.name}></DeleteButton>
      </li>
    )
  }
...
const DeleteButton = (props) => {
  return (
    <button onClick={() => remove(props.id, props.name)} id={props.id}>
      Delete
    </button>
  )
}
